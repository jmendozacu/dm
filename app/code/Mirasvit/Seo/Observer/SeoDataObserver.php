...
class SeoDataObserver implements ObserverInterface
{
     ...
     * @var string
     */
    protected $categoryImageTemplate = 'Mirasvit_Seo::category/image.phtml';
    ...
    public function __construct(
        ...
        \Mirasvit\Seo\Api\Service\DesignServiceInterface $designService,
    ...
    public function applyMeta(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if ($seo->getMetaTitle() && !$this->designService->isAllowedThemeForUpdateMeta()) {
    ...
    public function categoryAttribute($outputHelper, $outputHtml, $params)
    {
        ...
        switch ($params['attribute']) {
            case 'name':
                ...
                break;
            case 'description':
                if ($categoryDescription = $seo->getCategoryDescription()) {
                    $outputHtml = $categoryDescription;
                }
                break;
            case 'image':
                if ($categoryDescription = $seo->getCategoryImage()) {
                    $outputHtml = $this->blockFactory
                        ->createBlock(\Mirasvit\Seo\Block\Category\Image::class)
                        ->setTemplate($this->categoryImageTemplate)
                        ->toHtml();
                }
                break;
        }
