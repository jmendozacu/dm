...
class Organization extends \Magento\Framework\View\Element\Template
{
    ...
    public function getName()
    {
        if ($this->config->getNameOrganizationSnippets($this->store->getStoreId())) {
            ...
            if ($storeName = $this->_storeInfo->getStoreInformationObject($this->store)->getName()) {
                ...
                $name = trim($this->context->getScopeConfig()->getValue(Information::XML_PATH_STORE_INFO_NAME));
            }
        }
        ...
        if ($this->config->getCountryAddressOrganizationSnippets($this->store->getStoreId())) {
            ...
            $countryAddress = trim($this->localeLists
                ->getCountryTranslation($this->context
                        ->getScopeConfig()
                        ->getValue(Information::XML_PATH_STORE_INFO_COUNTRY_CODE)));
        }
    ...
    public function getAddressLocality()
    {
        if ($this->config->getLocalityAddressOrganizationSnippets($this->store->getStoreId())) {
            ...
            $addressLocality = trim($this->context->getScopeConfig()->getValue(Information::XML_PATH_STORE_INFO_CITY));
        }
    ...
    public function getPostalCode()
    {
        if ($this->config->getPostalCodeOrganizationSnippets($this->store->getStoreId())) {
            ...
            $postalCode = trim($this->context->getScopeConfig()->getValue(Information::XML_PATH_STORE_INFO_POSTCODE));
        }
    ...
    public function getStreetAddress()
    {
        if ($this->config->getStreetAddressOrganizationSnippets($this->store->getStoreId())) {
            ...
            $streetAddress = trim($this->context->getScopeConfig()->getValue(Information::XML_PATH_STORE_INFO_STREET_LINE1).
                ' '.$this->context->getScopeConfig()->getValue(Information::XML_PATH_STORE_INFO_STREET_LINE2));
        }
    ...
    {
        $addressRegion = false;
        if ($this->config->getAddressRegionOrganizationSnippets($this->store->getStoreId())) {
            $addressRegion = $this->config->getManualAddressRegionOrganizationSnippets($this->store->getStoreId());
        } elseif ($regionId = $this->context->getScopeConfig()->getValue(Information::XML_PATH_STORE_INFO_REGION_CODE)) {
            $addressRegion = $this->getRegionCodeById($regionId);
        }

        if ($addressRegion) {
            $addressRegion = SnippetsTrait::prepareSnippet($addressRegion);
            return "\"addressRegion\": \"$addressRegion\",";
        }
    ...
    public function getTelephone()
    {
        if ($this->config->getTelephoneOrganizationSnippets($this->store->getStoreId())) {
            ...
            $telephone = trim($this->context->getScopeConfig()->getValue(Information::XML_PATH_STORE_INFO_PHONE));
        }
     ...
     * @return string
     */
    public function preparePostalAddress($countryAddress,
                                         $addressLocality,
                                         $postalCode,
                                         $streetAddress,
                                         $addressRegion
    ) {
        $postalAddress = $countryAddress.$addressLocality.$postalCode.$streetAddress.$addressRegion;
