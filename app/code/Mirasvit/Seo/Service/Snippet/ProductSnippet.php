...
class ProductSnippet implements ProductSnippetInterface
{
    ...
    public function getProductSnippets()
    {
        ...
        if ($product = $this->registry->registry('current_product')) {
            $poductSnippets =
                ...
                . $this->getGtin($product, 8)
                . $this->getGtin($product, 12)
                . $this->getGtin($product, 13)
                . $this->getGtin($product, 14);
        }
    ...
    public function getImage($product)
    {
        ...
        if (strpos($image, '/placeholder/') !== false) {
            $image = ',"image":"' . SnippetsTrait::prepareSnippet(
                    $this->imageService->getProductBaseImageUrlByProduct($product, 'product_base_image')->getImageUrl()
                ) . '"';
        }
     ...
     * @return string
     */
    public function getGtin($product, $number)
    {
        $gtin = '';
        $attributeGtin = false;
        $function = 'getRichSnippetsGtin' . $number . 'Attributes';

        if ($gtinPrepared = $this->config->$function()) {
            $attributeGtin = $this->getRichSnippetsAttributeValue($gtinPrepared, $product);
        }
        if ($attributeGtin) {
            $attributeGtin = SnippetsTrait::prepareSnippet($attributeGtin);
            $gtin = ',"gtin' . $number . '":"' . SnippetsTrait::prepareSnippet($attributeGtin) . '"';
        }
