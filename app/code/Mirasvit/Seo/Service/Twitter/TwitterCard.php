...
class TwitterCard implements TwitterCardInterface
{
     ...
     * @param \Mirasvit\Seo\Api\Service\Image\ImageServiceInterface $imageService
     */
    public function __construct(
        ...
        \Mirasvit\Seo\Api\Service\Image\ImageServiceInterface $imageService
    ) {
        $this->config = $config;
            ...
            case 'catalog_product_view':
                ...
                if ($product && $product->getData('image') != 'no_selection') {
                    ...
                    $mediumImageUrl = $this->catalogImage
                                           ...
                                           ->getUrl();
                    $mediumImageUrl = $this->prepareImageUrl($mediumImageUrl, $product);
                ...
                if ($productCollection = $this->registry->registry(CurrentPageProductsInterface::PRODUCT_COLLECTION)) {
                    if ($productCollection->count()) {
                        $baseImageUrl = $this->catalogImage
                                             ...
                                             ->getUrl();
                        $baseImageUrl = $this->prepareImageUrl($baseImageUrl, $productCollection->getFirstItem());
     ...
     * @return string
     */
    protected function prepareImageUrl($imageUrl, $product)
    {
        if (strpos($imageUrl, '/placeholder/') !== false) {
            $imageUrl = $this->imageService->getProductBaseImageUrlByProduct(
                $product,
                'product_base_image'
            )->getImageUrl();
        }
