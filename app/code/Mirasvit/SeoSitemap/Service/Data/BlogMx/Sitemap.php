...
class Sitemap implements \Mirasvit\SeoSitemap\Api\Data\BlogMx\SitemapInterface
{
    ...
    {
        /** @var \Mirasvit\Blog\Model\Config $config */
        $config = $this->objectManager->get('\Mirasvit\Blog\Model\Config');
        ...
        foreach ($postCollectionFactory as $post) {
            $postCollection[] = new \Magento\Framework\DataObject(
                [
                    'name' => $post->getName(),
                    'url'  => $this->getBaseRoute() . '/' . $post->getUrlKey() . $config->getPostUrlSuffix(),
                ]
            );
        }
